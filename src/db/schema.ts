...
export const vaultItemsRelations = relations(vaultItems, ({ one }) => ({
  ...
  folder: one(folders, { fields: [vaultItems.folderId], references: [folders.id] }),
  secret: one(vaultSecrets, { fields: [vaultItems.id], references: [vaultSecrets.itemId] }),
}));

export const vaultSecretsRelations = relations(vaultSecrets, ({ one }) => ({
  item: one(vaultItems, { fields: [vaultSecrets.itemId], references: [vaultItems.id] }),
